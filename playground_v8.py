...
def search_wikidata_concepts(keyword,
    ...
    # Ensure failed_keywords is a dictionary
    if failed_keywords is None:
        failed_keywords = {}
    # Attempt to fetch the concepts
    retries = 0
    while retries < max_retries:
        try:
            ...
            if response.status_code == 200:
                ...
                if not results:
                    # No IDs found, log keyword as failed
                    if sentence_idx is not None:
                        failed_keywords.setdefault(sentence_idx, []).append(keyword)
                    else:
                        failed_keywords.setdefault("global", []).append(keyword)
                return [result['id'] for result in results]  # Return Wikidata IDs
            print(f"Error fetching concepts for keyword: {keyword}, Status: {response.status_code}")
            if sentence_idx is not None:
                failed_keywords.setdefault(sentence_idx, []).append(keyword)
            else:
                failed_keywords.setdefault("global", []).append(keyword)
...
def wikidata_main_entities(text):
    failed_keywords = {}  # Initialize as a dictionary
    ...
    for i, sentence in enumerate(sentences):
        ...
        for keyword in keywords:
            ids = search_wikidata_concepts(keyword, failed_keywords, sentence_idx=i)  # Pass as dictionary
            if not ids:
                print(f"No Wikidata match for keyword '{keyword}'. Skipping...")
                continue
            # Sort by the number after 'Q'
            sorted_ids = sorted(ids, key=lambda x: int(x[1:])) if ids else []
            # Check if sorted_ids is not empty
            if sorted_ids:
    ...
    return wikidata_selected_concepts, failed_keywords
...
def semantic_enrichment(triples_per_sentence,
                        failed_keywords):
    # Objects to store information and a set to avoiding duplicates
    enriched_triples_per_sentence = {}
    all_enriched_triples = []
    seen_triples = set()  
    # Enrich triples generated from the main entities
    for sentence_idx, sentence_triples in triples_per_sentence.items():
        enriched_triples = []  # Store enriched triples for this sentence
            ...
            for synset in synsets:
                ...
                for synonym in synonyms:
                    ...
                    if new_triple not in seen_triples:
                        ...
                        print(f"Added enriched triple: {new_triple}")
        # Store the enriched triples for this sentence
        enriched_triples_per_sentence[sentence_idx] = enriched_triples
    # Enrich failed keywords
    print("\nEnriching failed keywords...")
    for sentence_idx, keywords in failed_keywords.items():
        if sentence_idx not in enriched_triples_per_sentence:
            enriched_triples_per_sentence[sentence_idx] = []  # Initialize if not present
        # Iterate over each failed keyword
        for keyword in keywords:
            # Find synonyms for the failed keyword
            synsets = wordnet.synsets(keyword)
            for synset in synsets:
                synonyms = synset.lemma_names()
                for synonym in synonyms:
                    new_triple = (keyword, "synonym", synonym)
                    if new_triple not in seen_triples:
                        seen_triples.add(new_triple)
                        all_enriched_triples.append(new_triple)
                        enriched_triples_per_sentence[sentence_idx].append(new_triple)
                        print(f"Added enriched triple for failed keyword: {new_triple}")
...
def textrank_algorithm_with_kg(text,
    ...
    pd.set_option('display.expand_frame_repr', False)  # Avoid wrapping to new lines
...
# ROUGE family
def evaluate_with_rouge(summary,
                        reference_text):
...
# BERTScore
def evaluate_with_bert_score(summary,
                             reference_text):
    ...
    assert len(summary) == len(reference_text), "Candidates and references must have the same length"
    # Calculate BERTScore
    P, R, F1 = bert_score(summary, reference_text, lang='en', verbose=True)
    return round(P.mean().item(), 3), round(R.mean().item(), 3), round(F1.mean().item(), 3)
...
def summarization(text,
    ...
    if add_wikidata_information == 'Yes':
        ...
        sentence_concepts, failed_concepts = wikidata_main_entities(text)
        ...
        semantics_per_sentence, semantic_triples = semantic_enrichment(initial_triples_per_sentence, failed_concepts)
        ...
        # Time
        final_time = time.time()
        return summary, rank_df, rouge_scores, mean_rouge, P, R, F1, (final_time - start_time)
...
if __name__ == "__main__":
    ...
    percentage_txt = 0.5  # NOT USED!
    
    # Generate the summary for a test text
    if text == 'Test':
        # Define way of summarization
        num_sentences = 3  # <------------------------------------------------------ TO CHOICE
        # Prepare a list to collect results
        test_final_results = []
        # Standard summary
        summary, rank_df, rouge_scores, mean_rouge, p, r, f1, t = summarization(test, num_sentences, percentage_txt, "Number of sentences", "No")
        # KG summary
        summary_kg, rank_df_kg1, rank_df_kg2, rouge_scores_kg, mean_rouge_kg, p_kg, r_kg, f1_kg, t_kg = summarization(test, num_sentences, percentage_txt, "Number of sentences", "Yes")
