...
# Function to get relationships of P31 and P279 for an entity
...
def get_entity_label(entity_id):
    query = f"""SELECT ?label WHERE {{wd:{entity_id} rdfs:label ?label.
                FILTER(LANG(?label) = "en")}}"""
    results = execute_sparql_query(query)
    if results and results["results"]["bindings"]:
        return results["results"]["bindings"][0]['label']['value']  # Return the label of the entity
    return "Label not found"


# Function to query relationships for multiple entities
    ...
    for entity_id in entity_ids:
        ...
        if related_entities:
            ...
            for related_entity_id, related_entity_label, property_id in related_entities:
                relation = "instance of" if property_id == 'P31' else "subclass of"
...
def wikidata_entities_relations(wikidata_selected_concepts):
    ...
    for sentence_idx, concept_ids in wikidata_selected_concepts.items():
...
def summ_1(text,
    ...
    if add_wikidata_information == 'yes':
        ...
        # print("\nSUMM - Extracted Wikidata concepts for each sentence:")
        # for i, concepts in sentence_concepts.items():
        #     print(f"Sentence {i}: {concepts}")

        extract_concepts = wikidata_entities_relations(sentence_concepts)
        # print("\nSUMM - Extracted Wikidata concepts + entities for each sentence:")
        # print(extract_concepts)

        all_entities, sorted_extracted = wikidata_all_entities(extract_concepts)
        # print("\nSUMM - All Wikidata entities:")
        # print(all_entities)
