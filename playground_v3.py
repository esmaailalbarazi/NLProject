...
def semantic_enrichment(cleaned_triples):
    # Start with the cleaned triples and store new semantic triples
    enriched_triples = cleaned_triples.copy()  
    additional_semantic_relationships = []
    # Iterate over each triple
    for subject, predicate, obj in cleaned_triples:
        # Query Wikidata for hierarchical relationships
        for relation in ["P279", "P361", "P31"]:  # Subclass, part of, instance of
            ...
            for related_entity_id, related_entity_label, property_id in new_relations:
                ...
                additional_semantic_relationships.append(new_triple)
                print(f"Added semantic relationship: {new_triple}")
        # Use WordNet to add synonyms or hypernyms
        synsets = wordnet.synsets(obj)
        for synset in synsets:
            synonyms = synset.lemma_names()
            for synonym in synonyms:
                if synonym != obj:  # Avoid duplicating the same object
                    new_triple = (subject, "synonym", synonym)
                    enriched_triples.append(new_triple)
                    additional_semantic_relationships.append(new_triple)
                    print(f"Added synonym relationship: {new_triple}")
        # Add a delay to prevent overloading external APIs
        time.sleep(3)
    print("\nEnriched Triples:")
    for triple in enriched_triples:
        print(triple)
    print("\nAdditional Semantic Relationships:")
    for triple in additional_semantic_relationships:
        print(triple)    
    return enriched_triples, additional_semantic_relationships
...
def summ_1(text,
    ...
    if add_wikidata_information == 'yes':
        ...
        # Call triples
        triples_per_sentence, all_triples = query_to_generate_triples(filtered_concepts)

        # Call semantic enrichment with WordNet
        enriched_triples, semantic_layer = semantic_enrichment(all_triples)

        print("\nAll tripes: ", len(all_triples))
        print("Semantic layer: ", len(semantic_layer))
        print("Enriched triples: ", len(enriched_triples))
        print("\n")
